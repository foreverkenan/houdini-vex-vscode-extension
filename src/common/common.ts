...
export function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(hoverProvider, configHandler);
}
...
async function GenFunctionsTxtFromVexZip(exeFilePath: string, houdiniVexZipPath: string, functionsFilePath: string, extensionUri: vscode.Uri): Promise<string> {
    const args = [`"${houdiniVexZipPath}"`, `"${path.join(extensionUri.fsPath, functionsFilePath)}"`];
